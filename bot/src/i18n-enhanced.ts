/**
 * Bot i18n module — Enhanced version
 * 
 ...
 * 
 * Enhanced features:
 * - Emoji registry integration for consistent emoji usage
 * - Rich message template support (bold, code, emoji entities)
 * - Pluralization helpers for Russian and other languages
 * - Date/time formatting with localization
 */

import { EMOJI, resolveEmoji, unicodeOf, EmojiEntry } from "./modules/emoji/registry.js";

// Re-export emoji registry for convenience
export { EMOJI, resolveEmoji, unicodeOf };
 ...
 * 
 * @param key - Translation key (e.g., "menu.btn_tariffs")
 * @param lang - Language code (default: "ru")
 * @param vars - Optional template variables to replace
...
// ═══════════════════════════════════════════════════════════════════════════════
// Emoji Integration Helpers
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Get emoji for a key, resolved through the emoji registry.
 * Supports premium emoji with fallback to Unicode.
 */
export function emoji(
  key: string,
  premiumOverrides?: Record<string, { unicode?: string; tgEmojiId?: string }> | null,
): string {
  return resolveEmoji(key, premiumOverrides).unicode;
}

/**
 * Create a translated message with emoji prefix.
 * Uses the emoji registry for consistent emoji usage.
 * 
 * @param emojiKey - Emoji key from registry (e.g., "STATUS_ACTIVE")
 * @param translationKey - Translation key for the message text
 * @param lang - Language code
 * @param vars - Template variables
 * @param premiumOverrides - Premium emoji overrides
 * @returns Message string with emoji prefix
 */
export function tWithEmoji(
  emojiKey: string,
  ...
  const emojiStr = resolveEmoji(emojiKey, premiumOverrides).unicode;
  const text = t(translationKey, lang, vars);
  return `${emojiStr} ${text}`;
}
...
/**
 * Format days with proper pluralization for Russian and English.
 */
...
/**
 * Pluralize a word based on count (Russian-specific).
 * Usage: pluralize(5, "день", "дня", "дней")
 */
...
export function formatMoney(amount: number | string, currency = "RUB"): string {
  ...
  return `${num.toLocaleString("ru-RU", { minimumFractionDigits: 0, maximumFractionDigits: 2 })} ${sym}`;
}

/**
 * Format traffic in bytes to human-readable GB/MB.
 */
 ...
 */
export function formatDate(date: Date | string | number, lang = "ru"): string {
...
/**
 * Format a date-time to localized string.
 */
export function formatDateTime(date: Date | string | number, lang = "ru"): string {
  const d = typeof date === "string" ? new Date(date) : typeof date === "number" ? new Date(date * 1000) : date;
...
// ═══════════════════════════════════════════════════════════════════════════════
// Rich Message Template Helpers
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Create a bold text segment for Telegram messages.
 */
...
/**
 * Create an inline code text segment.
 */
 ...
 * Example: "Balance: 1000 ₽" → "💰 *Balance:* 1000 ₽"
 */
export function kvLine(
  ...
  const prefix = emojiKey ? resolveEmoji(emojiKey, premiumOverrides).unicode + " " : "";
 ...
 */
export function separator(char = "─", length = 20): string {
  return char.repeat(length);
}

/**
 * Build a status line with emoji.
 * Example: statusLine("active", "Subscription", "Expires: 2024-01-01")
 */
export function statusLine(
  status: "active" | "expired" | "inactive" | "limited" | "disabled",
  label: string,
  details?: string,
): string {
