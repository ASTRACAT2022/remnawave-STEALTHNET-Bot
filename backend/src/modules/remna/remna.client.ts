...
function normalizeRemnaBaseUrl(value: string | undefined): string {
  if (!value) return "";
  return value
    .trim()
    .replace(/\/+$/, "")
...
const REMNA_API_URL = normalizeRemnaBaseUrl(env.REMNA_API_URL);
...
export async function remnaFetch<T>(
  ...
  const url = `${REMNA_API_URL}${urlPath}`;
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), env.REMNA_TIMEOUT_MS);
  ...
  try {
    ...
    if (e instanceof Error && e.name === "AbortError") {
      return { error: `Remna API timeout after ${env.REMNA_TIMEOUT_MS}ms (${url})`, status: 504 };
    }
