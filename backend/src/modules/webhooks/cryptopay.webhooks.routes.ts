...
cryptopayWebhooksRouter.post("/", async (req: Request, res: Response) => {
  recordPaymentWebhookReceived("cryptopay");
  ...
  if (!rawString) {
    ...
    recordPaymentWebhookOutcome("cryptopay", "ignored");
  ...
  if (!verifyCryptopayWebhookSignature(token, rawString, signature)) {
    ...
    recordPaymentFailed("cryptopay", "signature_invalid");
    recordPaymentWebhookOutcome("cryptopay", "rejected_signature");
  ...
  try {
    ...
    recordPaymentWebhookOutcome("cryptopay", "bad_payload");
    return res.status(200).send("OK");
  }
  ...
  if (!payment) {
    ...
    recordPaymentFailed("cryptopay", "payment_not_found");
    recordPaymentWebhookOutcome("cryptopay", "payment_not_found");
    return res.status(200).send("OK");
  }
  ...
  if (payment.status === "PAID") {
    recordPaymentWebhookOutcome("cryptopay", "payment_already_paid");
    return res.status(200).send("OK");
  }
  ...
  recordPaymentProcessed("cryptopay", deriveProduct(payment), seconds);
  recordPaymentWebhookOutcome("cryptopay", "accepted");
  return res.status(200).send("OK");
});
