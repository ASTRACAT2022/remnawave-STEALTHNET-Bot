...
adminRouter.patch("/payments/:id", asyncRoute(async (req, res) => {
  ...
  const now = new Date();
  const isTopUp = (
    payment.provider === "yoomoney_form"
...
adminRouter.patch("/settings", async (req, res) => {
  ...
  if (updates.telegramStarsEnabled !== undefined) {
    await prisma.systemSetting.upsert({
      where: { key: "telegram_stars_enabled" },
      create: { key: "telegram_stars_enabled", value: updates.telegramStarsEnabled ? "true" : "false" },
      update: { value: updates.telegramStarsEnabled ? "true" : "false" },
    });
  }
  if (updates.telegramStarsRate !== undefined) {
    await prisma.systemSetting.upsert({
      where: { key: "telegram_stars_rate" },
  ...
  if (updates.botAdminIds !== undefined) {
    const val = (updates.botAdminIds ?? "")
      .split(/[,\n;]+/)
      .map((v) => v.trim())
      .filter((v) => /^\d+$/.test(v))
      .filter((v, i, arr) => arr.indexOf(v) === i)
      .join(",");
    await prisma.systemSetting.upsert({ where: { key: "bot_admin_ids" }, create: { key: "bot_admin_ids", value: val }, update: { value: val } });
  }
