...
export async function activateTariffByPaymentId(paymentId: string): Promise<ActivationResult> {
  ...
  // 2. isAdditionalSubscription → createAdditionalSubscription (явный запрос новой доп.)
  // 3. Есть существующая подписка у клиента → extendSecondarySubscription (стек дней)
  // 4. Нет подписок → createAdditionalSubscription (первая покупка, subscriptionIndex=0)
  //
  // Эффект: «обычная покупка» продлевает существующую подписку (stack дней), а не
  // плодит новых подписок с суффиксами _0, _1, _2. Хочешь новую — передай
  ...
  if (payment.tariffId) {
    ...
    // ── Ветка 2: продление существующей подписки или создание новой ──
    const isAdditional = isAdditionalSubscriptionPayment(payment.metadata);

    // Подарочная покупка ИЛИ явный запрос новой доп. подписки → всегда создаём новую
    if (isGiftPurchase || isAdditional) {
...
/** metadata.isAdditionalSubscription=true → пользователь явно запросил НОВУЮ доп. подписку (не продление). */
