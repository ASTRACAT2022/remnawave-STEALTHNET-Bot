...
STEALTHNET API → POST /api/keys → WDTT-нода
        │
        ▼
Нода создаёт уникальный пароль → возвращает wdtt:// ссылку
        │
        ▼
...
1. Откройте админку: `https://your-domain.com/admin/olcrtc`
2. Перейдите в **«Ноды»** → **«Добавить ноду»**
3. В поле **«Режим выдачи»** выберите **«WDTT-совместимая нода»** и заполните:

| Поле | Значение |
|------|----------|
| Название | `Node #1` |
| URL WDTT API | `http://YOUR_WDTT_IP:9000` |
| API Key | ваш ключ из `.env` |
| DTLS Port | `56000` |
| WG Port | `56001` |
| Capacity | `100` (или пусто) |

4. Нажмите **«Создать»**, затем у ноды нажмите **«Тест»**. Только после
   успешного теста нода перейдёт в `ONLINE` и сможет принимать оплаты.

Нода находится в общем разделе OlcRTC только потому, что прежние тарифы и
платежи используют исторические таблицы `wdtt_*`. Это не меняет протокол:
такая нода выдаёт клиентам именно `wdtt://`, а персональные OlcRTC-ноды —
именно `olcrtc://`.

### Массовый перенос действующих OlcRTC-доступов в WDTT

После успешного теста WDTT-ноды откройте **OlcRTC → Подписки**, нажмите
**«Выбрать всех»**, затем **«Перенести выбранные»** и выберите WDTT-ноду.
Для каждого действующего оплаченного доступа биллинг создаст отдельный
`wdtt://`-ключ. Срок действия, оплата и использованный трафик сохраняются,
а прежняя ссылка OlcRTC остаётся в резервной копии. Истёкшие и отозванные
доступы намеренно не переносятся.
