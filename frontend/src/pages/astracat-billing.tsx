...
export function AstracatBillingPage() {
  ...
  const [theme, setTheme] = useState<"light" | "dark">("dark"); const [collapsed, setCollapsed] = useState(false); const [mobileMenuOpen, setMobileMenuOpen] = useState(false); const [tabs, setTabs] = useState<PageId[]>(savedTabs); const [active, setActive] = useState<PageId>(() => savedTabs()[0]); const [modal, setModal] = useState<ModalName>(null); const [notice, setNotice] = useState<string | null>(null); const [ticketToOpen, setTicketToOpen] = useState<string | null>(null);
  ...
  const openTicket = (id: string) => { open("tickets"); setTicketToOpen(id); };
...
function BillingPage({ page, data, token, open, modal, inform, refreshProfile, ticketToOpen, onTicketHandled }: { page: PageId; data: ReturnType<typeof useBillingData>; token: string; open: (page: PageId) => void; modal: (name: ModalName) => void; inform: (message: string) => void; refreshProfile: () => Promise<void>; ticketToOpen: string | null; onTicketHandled: () => void }) {
  ...
  if (page === "tickets" || page === "archive") return <Tickets tickets={data.tickets.filter((ticket) => page === "archive" ? ticket.status === "closed" : ticket.status !== "closed")} token={token} modal={modal} reload={data.reload} ticketToOpen={ticketToOpen} onTicketHandled={onTicketHandled} />;
...
function Tickets({ tickets, token, modal, reload, ticketToOpen, onTicketHandled }: { tickets: TicketRow[]; token: string; modal: (name: ModalName) => void; reload: () => Promise<void>; ticketToOpen: string | null; onTicketHandled: () => void }) {
  const [chatId, setChatId] = useState<string | null>(null);
  useEffect(() => {
    if (!ticketToOpen) return;
    setChatId(ticketToOpen);
    onTicketHandled();
  }, [ticketToOpen, onTicketHandled]);
  return <Frame title="Запросы" actions={<Button onClick={() => modal("ticket")}><Plus size={16} /> Создать запрос</Button>}><div className="ac-table-wrap"><table className="ac-table"><thead><tr><th>Тема</th><th>Статус</th><th>Обновлён</th><th aria-label="Действие" /></tr></thead><tbody>{tickets.map((ticket) => <tr key={ticket.id} className="ac-row-click" title="Открыть переписку" onClick={() => setChatId(ticket.id)}><td><strong>{ticket.subject}</strong></td><td><StatusBadge tone={ticket.status === "closed" ? "neutral" : "success"}>{ticket.status}</StatusBadge></td><td>{date(ticket.updatedAt)}</td><td><Button tone="secondary" className="ac-ticket-open" onClick={(event) => { event.stopPropagation(); setChatId(ticket.id); }}>Открыть чат</Button></td></tr>)}</tbody></table></div>{tickets.length === 0 && <EmptyState title="Обращений нет" text="Создайте запрос — он будет сохранён в системе поддержки." />}{chatId && <TicketChatModal key={chatId} token={token} ticketId={chatId} onClose={() => setChatId(null)} onUpdated={() => { void reload(); }} />}</Frame>;
}
