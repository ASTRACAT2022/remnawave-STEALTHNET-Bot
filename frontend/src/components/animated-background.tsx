...
const ORBS: OrbConfig[] = [
  { x: 0.15, y: 0.20, r: 360, dx: 120, dy: 90,  dur: 14 },
  { x: 0.80, y: 0.10, r: 320, dx: -100, dy: 110, dur: 18 },
  { x: 0.60, y: 0.70, r: 400, dx: -130, dy: -100, dur: 22 },
  { x: 0.10, y: 0.80, r: 300, dx: 90,  dy: -110, dur: 16 },
  { x: 0.90, y: 0.55, r: 340, dx: -90, dy: 100,  dur: 20 },
  { x: 0.45, y: 0.40, r: 280, dx: 100, dy: -90,  dur: 26 },
];
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      for (let i = 0; i < activeOrbs.length; i++) {
        ...
        const cx = (o.x * W) + Math.sin(phase) * scaledDx + Math.sin(t * 0.3 + i) * 20 * scale;
        const cy = (o.y * H) + Math.cos(phase * 0.7 + i * 1.2) * scaledDy + Math.cos(t * 0.4 + i * 2) * 15 * scale;
      ...
      if (!lowPower) {
        ctx.save();
        ctx.globalAlpha = 0.03 * scale;
        ctx.strokeStyle = orbColors[0].center.replace(/[\d.]+\)$/, "0.04)");
        ctx.lineWidth = 1;
        for (let w = 0; w < 3; w++) {
          ctx.beginPath();
          const wy = H * (0.25 + w * 0.25) + Math.sin(t * 0.2 + w * 1.5) * 15 * scale;
          for (let x = 0; x <= W; x += 4) {
            const y = wy + Math.sin(x * 0.008 + t * 0.3 + w * 2) * 8 * scale + Math.sin(x * 0.015 + t * 0.5 + w) * 4 * scale;
