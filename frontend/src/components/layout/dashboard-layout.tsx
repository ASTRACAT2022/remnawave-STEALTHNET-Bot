            ...
            {groupedNav[category].map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={onClick}
                  className={cn(
                    "flex items-center gap-3.5 py-2.5 px-3 rounded-lg transition-colors duration-150 relative border-l-[3px] border-r-0",
                    isActive
...
export function DashboardLayout() {
  ...
  return (
    <div className="admin-fast-ui flex min-h-svh bg-background relative">

      {/* ═══ Desktop sidebar ═══ */}
      <aside className="hidden md:flex flex-col shrink-0 fixed left-0 top-0 bottom-0 w-[290px] z-[70] border-r border-border bg-card shadow-sm overflow-hidden">
        <div className="flex h-16 items-center justify-center gap-3 px-4 relative z-10">
          <div className="absolute bottom-0 left-4 right-4 h-px bg-border"></div>
          {brand.logo ? (
        ...
        <nav className="flex-1 space-y-1.5 p-4 overflow-y-auto relative z-10 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent">
          <NavItems />
        </nav>
        <div className="border-t border-border p-4 space-y-1.5 relative z-10">
          <div className="text-[12px] font-mono font-bold text-emerald-500 uppercase tracking-widest px-3 py-1 mb-1 flex items-center gap-2">
          ...
          <Button 
            variant="ghost" 
            size="sm" 
            className="w-full justify-start gap-2 text-red-500/80 hover:bg-red-500/10 hover:text-red-500 transition-colors font-mono font-bold text-[13px] mt-1" 
      ...
      {mobileMenuOpen && (
        <>
          <div
            className="fixed inset-0 z-[69] bg-black/40 md:hidden animate-in fade-in duration-150"
            onClick={() => setMobileMenuOpen(false)}
          />
          <aside className="fixed left-0 top-0 bottom-0 z-[70] w-[290px] flex flex-col md:hidden bg-card border-r border-border shadow-xl overflow-hidden animate-in slide-in-from-left duration-150">
      ...
      <main className="flex-1 min-w-0 flex flex-col md:pl-[290px] w-full relative z-10">
        <header className="sticky top-0 z-[70] flex h-16 shrink-0 items-center justify-between gap-3 border-b border-border bg-card px-4 md:px-5 shadow-sm">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <div className="hidden md:flex h-9 w-9 rounded-xl bg-primary/10 border border-primary/20 items-center justify-center shrink-0">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <div className="relative">
              <Button variant="ghost" size="sm" className="gap-1.5 text-xs h-9 px-2.5 rounded-xl border border-border bg-background hover:bg-muted" onClick={() => setShowThemePanel(!showThemePanel)}>
              ...
              {showThemePanel && (
                <>
                  ...
                  <div className={cn(
                    "absolute right-0 top-full z-50 mt-3 w-[calc(100vw-2rem)] sm:w-[320px] max-w-[320px] rounded-2xl border border-border bg-card p-5 shadow-xl transition-opacity duration-150 origin-top-right",
                        ...
                        {MODE_OPTIONS.map((opt) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "flex flex-1 items-center justify-center gap-2 rounded-lg py-2 text-xs font-medium transition-colors duration-150",
                                isActive
                        ...
                        {(Object.entries(ACCENT_PALETTES) as [ThemeAccent, typeof ACCENT_PALETTES["default"]][]).map(([key, palette]) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "group flex flex-col items-center gap-2 rounded-xl p-2 transition-colors duration-150",
                                isActive ? "bg-primary/10" : "hover:bg-muted/60"
                              )}
                            >
                              <div
                                className={cn(
                                  "relative flex h-8 w-8 items-center justify-center rounded-full shadow-sm",
            ...
            <a href={GITHUB_URL} target="_blank" rel="noopener noreferrer"
              className="hidden sm:flex items-center gap-1.5 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5 text-xs font-medium text-emerald-700 dark:text-emerald-400 transition-colors hover:bg-emerald-500/15">
        ...
        <div className="flex-1 px-4 md:px-6 pt-5 pb-6 animate-in fade-in duration-150 relative z-10">
