...
class SubscriptionService:
    ...
    @staticmethod
    def _is_not_found_api_error(error: RemnaWaveAPIError) -> bool:
        ...
        return any(marker in normalized for marker in ('not found', 'specified params', 'specified parameter'))
        ...
        try:
            ...
            async with self.get_api_client() as api:
                ...
                if not existing_users and user.telegram_id:
                    ...
                    except RemnaWaveAPIError as lookup_error:
                        if self._is_not_found_api_error(lookup_error):
                            existing_users = []
                        else:
                            logger.warning(
                                '⚠️ Ошибка поиска пользователя в панели по telegram_id, продолжаем создание',
                ...
                if existing_users:
                    ...
                    except RemnaWaveAPIError as update_error:
                        if not self._is_not_found_api_error(update_error):
                            raise
                        logger.warning(
                            '⚠️ Найденный пользователь в панели недоступен для update, создаем нового',
                            _format_user_log=self._format_user_log(user),
                        )
                        updated_user = await api.create_user(**_build_create_kwargs())
            ...
            if self._is_not_found_api_error(e):
                logger.warning(
                    'RemnaWave пользователь не найден при обновлении, выполняем пересоздание',
